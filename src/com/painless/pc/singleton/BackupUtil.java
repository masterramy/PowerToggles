...
public class BackupUtil {
  ...
  public static BackupData readSettings(File importFile, Context context, int widgetId) throws Exception {
    ...
    try {
      ...
      if (settingsLine == null) {
        throw new Exception("Missing widget settings");
      }
      ...
      if (ids.length == 0) {
        throw new Exception("No trackers to import");
      }
      ...
      for (int i = 0; i<8 && i<ids.length; i++) {
        ...
        if (id.startsWith("ss_")) {
          String intentLine = reader.readLine();
          String folderName = reader.readLine();
          if (intentLine == null || folderName == null) {
            throw new Exception("Incomplete shortcut backup data");
          }
          ...
          if (intentLine == null || label == null) {
            throw new Exception("Incomplete plugin backup data");
          }
        ...
        if (trackers[i] == null) {
          throw new Exception("Unsupported tracker in backup");
        }
        ...
        if (iconEntry != null) {
          ...
          if (importedIcon == null) {
            throw new Exception("Invalid or oversized widget icon");
          }
          allIcons[i] = BitmapUtils.resizeToIconSize(importedIcon, context, false);
        }
      }
      if (newTrackerList.length() == 0) {
        throw new Exception("No valid trackers in backup");
      }
      ...
      if (backImage != null) {
        ...
        if (back == null) {
          throw new Exception("Invalid or oversized widget background");
        }
        ...
        if (targetWidth > MAX_SCALED_BACKGROUND_DIMENSION
            || targetHeight > MAX_SCALED_BACKGROUND_DIMENSION
            || targetWidth * targetHeight > MAX_SCALED_BACKGROUND_PIXELS) {
          back.recycle();
          throw new Exception("Scaled widget background is too large");
        }
        data.backImage = Bitmap.createScaledBitmap(back, (int) targetWidth, (int) targetHeight, true);
      ...
      // Only write embedded folders after all widget settings and images validate.
  ...
  private static byte[] readZipEntry(ZipFile zip, ZipEntry entry, long maxBytes, boolean required) throws Exception {
    if (entry == null) {
      if (required) {
        throw new Exception("Required backup entry missing");
      }
      return null;
    }
    InputStream in = null;
  ...
  public static long copy(InputStream in, OutputStream out, long maxBytes) throws Exception {
    if (in == null || out == null) {
      throw new Exception("Unable to copy null stream");
    }
    ...
    while ((bytesRead = in.read(bucket)) != -1) {
      total += bytesRead;
      if (total > maxBytes) {
        throw new java.io.IOException("Input exceeds maximum supported size");
      }
